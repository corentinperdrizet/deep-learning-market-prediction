...
"""
Train and evaluate the Transformer time-series classifier end-to-end.

Usage (defaults: BTC-USD, 1d, start=2018-01-01, test_start=2023-01-01):
    python -m src.training.run_transformer
You can override:
    python -m src.training.run_transformer --ticker ETH-USD --d-model 64 --n-layers 2
"""
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Prefix for artifact filenames (default: 'transformer' when ticker=BTC-USD, "
        "else 'transformer_<ticker>' so multi-asset runs don't collide)",
    )
...
def train_transformer(
    ...
    artifact_prefix: str = "transformer",
) -> dict:
    """Same shape/contract as src.training.run_lstm.train_lstm(): prepare
    data, train with early stopping via the shared trainer.fit() (AdamW, to
    match this model's documented optimizer), evaluate on test, persist a
    checkpoint that's self-describing via model_config, and a test report
    with the same schema as the LSTM's (meta/features/seq_len/seed/...).
    """
...
def main():
    ...
    prefix = args.artifact_prefix or default_artifact_prefix("transformer", args.ticker)
